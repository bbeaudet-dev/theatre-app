...
export const createTheatreCloudLayout = (rankings: any[]): PositionedShow[] => {
  // Safety check for empty rankings array
  if (!rankings || rankings.length === 0) {
    return [];
  }
  
  const sortedRankings = [...rankings].sort((a, b) => (a.rank || 0) - (b.rank || 0)); // Sort by rank (1 = best)
  ...
  for (const ranking of sortedRankings) {
    if (!ranking.show) continue;
    
    const baseWidth = getBaseSize(ranking.rank, rankings.length);
    
    // Safety check for valid width
    if (!baseWidth || baseWidth <= 0 || !isFinite(baseWidth)) {
      console.warn(`Invalid width calculated for show ${ranking.show.title}: ${baseWidth}`);
    ...
    if (!height || height <= 0 || !isFinite(height)) {
      console.warn(`Invalid height calculated for show ${ranking.show.title}: ${height}`);
    ...
    if (!position || !isFinite(position.x) || !isFinite(position.y) || !isFinite(position.width) || !isFinite(position.height)) {
      console.warn(`Invalid position calculated for show ${ranking.show.title}:`, position);
      continue; // Skip this show if position is invalid
    }
    
    positioned.push({
      show: ranking, // Store the full ranking object
      x: position.x,
      y: position.y,
...
// Function to get object-position for specific shows (optional, can be expanded)
export const getObjectPosition = (showTitle: string) => {
